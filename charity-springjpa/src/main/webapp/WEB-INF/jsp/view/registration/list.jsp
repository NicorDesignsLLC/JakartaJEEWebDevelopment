	...
	<c:choose>
		...
		<c:otherwise>
			<c:forEach items="${registrations}" var="registration">
                ...
                <a
					href="<c:url value='/registration/view/${registration.id}' />">
					<c:out
						value="${nicordesigns:abbreviateString(registration.subject, 60)}" />
				</a>
				<br />
				<c:out value="${registration.userName}" />&nbsp;
                <spring:message code="message.registrationList.created" />&nbsp;
                <nicordesigns:formatDate
					value="${registration.dateCreated}" type="both" timeStyle="short"
					dateStyle="medium" />
				<br />
				<br />
			</c:forEach>
